...
/*
    요구사항 분석.
**Client는 사용자가 종료하기 전까지 server로 메세지를 전달합니다.
-> 구현완료했음. while loop나 do while문을 활용해서 

**Server는 연결된 client가 종료되고 난 이후에 종료하지 않고 연결된 client의 연결 종료를 표시합니다.
-> 이것도 어느정도 구현완료, 그러나 sigint로 한번 진행해보겠음. 


**Server는 연결된 client가 종료되고 난 이후에 다른 client의 연결을 받을 수 있도록 합니다.
-> 이부분 어떻게 구현할지 고민.
첫번째로 클라이언트를 그냥 끄지 않은 상태로 둔 후 
두번째로 서버에서는 따로 사용자에게 입력을 받아서 끄면 될 것같다.
-> 두명과의 연결을 한다는 것이니. 쓰레드까지는 잘모르겠고.. 
   ...
   if (tcp_socket < 0) {
    ...
    while(true){
        ...
        if(client_socket <0){
            ...
            // close(tcp_socket); <-종료해도 연결은 유지해야함.
            // return 1;
            continue;
        }
