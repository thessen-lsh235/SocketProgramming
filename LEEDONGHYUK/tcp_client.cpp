...
int main()
{
    ...
    cout << "Client : 서버에 연결 되었습니다 " << endl;

    // 🌍 4. 서버로 데이터 전송하기

    const char *message = "Client : Hi Server!";
    if (send(tcp_socket, message, strlen(message), 0) < 0)
    {
        cerr << "Client : 메시지 전송 실패" << endl;
        close(tcp_socket);
        return 1;
    }
    std::cout << "Client : 메시지 전송 완료: " << message << std::endl;
